    ...
    return (
        <div className="p-4 flex flex-col">
            <div className="w-full flex justify-between">
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent>
                        ...
                        >
                            Aggiungi {isAddPending && <FaSpinner className="animate-spin duration-400"/>}
                        </Button>
                    </DialogContent>
                </Dialog>
            </div>
            ...
            <div className="overflow-auto grid grid-cols-4 auto-rows-min gap-2 h-full py-4 relative">
                {
                    !isLoading ?
                        posts.length == 0 ? <p>Non sono ancora state caricate notizie</p> : (posts.toReversed().map((post: { id: number, title: string, description: string, link: string }, index: number) => (
                        <Card className="h-fit" key={index}>
                            ...
                            <CardFooter className="flex-col items-start">
                                ...
                                <Button className="w-full mt-3 flex items-center gap-2" variant="destructive"
                                    onClick={() => {
                                        setRemoveId(post.id)
                                        removePost({ id: post.id })
                                    }}
                                    disabled={(removeSuccess || isRemovePending) && removeId == post.id}
                                >
                                    Rimuovi {(removeSuccess || isRemovePending) && removeId == post.id && <FaSpinner className="animate-spin duration-400"/>}
                                </Button>
                            </CardFooter>
                        </Card>))
